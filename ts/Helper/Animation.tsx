...
export class MAnim extends React.Component<IMAnimProps, IMAnimState> {
	dummy: HTMLDivElement | null = null;
	...
	render() {
		const children = this.state.data.map(d=>{
			...
			if (d.height) {
				return <Motion key={d.id} defaultStyle={{opac: 0,height: 0}} style={{opac: spring(1),height: spring(d.height)}}>{
					(obj)=>{
						return <div style={{
								height: obj.height,
								paddingBottom: 20,
				...
				return <div key={currentID} ref={r=>{
					if (r) {
						const height = r.getBoundingClientRect().height;
						this.setState({
							data: this.state.data.map(d=>(d.id==currentID?{
								...d,
								height
							}:d))
						});
					}
				}} style={{position: "absolute", top: 0, left: 0, width: "100%", paddingBottom: 20, visibility: "hidden"}}>
					{child}
				</div>
			}
		});
...
export class HAnim extends React.Component<IHAnimProps, IHAnimState> {
	...
	static getDerivedStateFromProps(props: IHAnimProps & {children: any}, state: IHAnimState) {
		...
		while (true) {
			if ((ci>=state.children.length) || (ui>=update.length)) {
				...
				for (; ui<update.length; ui++) {
					const found = _.find(state.children, c=>c.key==update[ui].key);
					if (found) {
						view.push({
							...found,
							status: "visible"
	...
	render() {
		return <div style={{
			...
			{this.state.children.map(child=>{
				return <HChild key={child.key} child={child} delChild={(key: any)=>{
					this.setState({
						children: this.state.children.filter(c=>c.key!=key)
					})
				}}/>
			})}
		</div>
...
class HChild extends React.Component<IHChildProps, IHChildState> {
	...
	static getDerivedStateFromProps(props: IHChildProps, state: IHChildState) {
		return {
			status: props.child.status
		}
	}
	render() {
		...
		return <Motion key={key} defaultStyle={{
			height: 0,
			opacity: 0
		}} style={{

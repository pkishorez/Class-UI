...
import { v4 } from 'uuid';
import { IFlashProps, Flash } from './_Flash';
import { IDrawerProps, Drawer } from './_Drawer';
...
export class Overlay extends React.Component<IProps, IState> {
	...
	static flash(options: IFlashProps) {
		...
		let id = v4();
		let fref: any;
		_instance.overlay(id, <Flash {...options} key={id} ref={r=>fref=r} remove={()=>{
	...
	static drawer(options: IDrawerProps) {
		...
		let id = v4();
		let dref: any;
		_instance.overlay(id, <Drawer {...options} key={id} ref={r=>dref=r} remove={()=>{
			Overlay.removeOverlay(id);
		}}/>);
	}

	static feedback: typeof Feedback["show"] = (content: string, type = "success", timeout=2)=> {
	...
	render() {
		...
		return <>{
			this.state.overlays.map(o=>o.component)
		}</>;
	}
}